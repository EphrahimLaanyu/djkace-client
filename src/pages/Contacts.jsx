...
const Contact = () => {
  ...
  useGSAP(() => {
    gsap.from(".form-track", { x: -30, opacity: 0, duration: 0.8, stagger: 0.1, ease: "power2.out", delay: 0.2 });

    const filmTween = gsap.to(".filmstrip", { yPercent: -50, ease: "none", duration: 10, repeat: -1 });
    ...
    const marqueeTween = gsap.to(".marquee-content", { xPercent: -100, repeat: -1, duration: 20, ease: "none" });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', width: '100%', overflowX: 'hidden' }}>
        <SEO title="Bookings & Contact" description="Book DJ Kace." url="https://deejaykace.co.ke/contacts" />
        ...
        <div style={styles.marqueeBand} ref={marqueeRef}>
            <div style={styles.marqueeTrack}>
                <span className="marquee-content" style={styles.marqueeText}>THANKS FOR VIBING • KEEP THE MUSIC LOUD • DEEJAY KACE • NAIROBI'S FINEST • ALL RIGHTS RESERVED •&nbsp;</span>
                <span className="marquee-content" style={styles.marqueeText}>THANKS FOR VIBING • KEEP THE MUSIC LOUD • DEEJAY KACE • NAIROBI'S FINEST • ALL RIGHTS RESERVED •&nbsp;</span>
            </div>
        </div>
        ...
        <style>{`
            ...
            @media (max-width: 900px) {
                .pageWrapper { flex-direction: column !important; }
                .info-side, .vertical-divider { display: none !important; }
                .sleeve-side { width: 100% !important; padding: 40px 20px !important; border-right: none !important; min-width: unset !important; }
...
const styles = {
  pageWrapper: { position: 'relative', width: '100vw', minHeight: '100vh', display: 'flex', flexDirection: 'row', backgroundColor: '#F1E9DB', color: '#111', fontFamily: '"Space Mono", monospace', overflowX: 'hidden', paddingBottom: '40px', className: 'pageWrapper' },
  marqueeBand: { position: 'absolute', bottom: 0, left: 0, width: '100%', height: '40px', backgroundColor: '#E60000', display: 'flex', alignItems: 'center', overflow: 'hidden', zIndex: 100, borderTop: '2px solid #111' },
  marqueeTrack: { display: 'flex', whiteSpace: 'nowrap', width: 'fit-content' },
  marqueeText: { fontSize: '1rem', fontWeight: '900', color: '#000', fontFamily: '"Rajdhani", sans-serif', letterSpacing: '2px', paddingRight: '50px', flexShrink: 0, display: 'block' },
  sleeveContainer: { flex: '1', padding: '80px 50px', display: 'flex', flexDirection: 'column', justifyContent: 'center', position: 'relative', minWidth: '320px', zIndex: 2, backgroundColor: '#F1E9DB' },
  ...
  rightSideContainer: { flex: '1', color: '#111', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'center', position: 'sticky', top: 0, height: '100vh', padding: '0', minWidth: '320px', zIndex: 1, overflow: 'hidden', backgroundColor: '#F1E9DB' },
  filmstrip: { width: '80%', display: 'flex', flexDirection: 'column', alignItems: 'center', willChange: 'transform' },
  filmItem: { width: '100%', marginBottom: '40px', display: 'flex', flexDirection: 'column', alignItems: 'center' },
  filmMeta: { width: '100%', display: 'flex', justifyContent: 'space-between', fontSize: '0.7rem', fontWeight: 'bold', color: '#E60000', marginBottom: '5px', fontFamily: '"Rajdhani", sans-serif' },
  filmNum: { fontWeight: '900', letterSpacing: '1px' },
  filmLine: { opacity: 0.3, letterSpacing: '-2px', overflow: 'hidden' },
  filmTag: { opacity: 0.8 },
  imageWrapper: { width: '100%', position: 'relative', padding: '0 20px', backgroundColor: '#111' },
  stripImage: { width: '100%', height: 'auto', display: 'block', filter: 'grayscale(100%) contrast(110%)' },
  sprocketLeft: { position: 'absolute', left: '5px', top: 0, bottom: 0, width: '10px', background: 'repeating-linear-gradient(to bottom, #fff 0px, #fff 10px, transparent 10px, transparent 20px)', opacity: 0.8 },
  sprocketRight: { position: 'absolute', right: '5px', top: 0, bottom: 0, width: '10px', background: 'repeating-linear-gradient(to bottom, #fff 0px, #fff 10px, transparent 10px, transparent 20px)', opacity: 0.8 },
  focusLine: { position: 'absolute', top: '50%', left: '0', right: '0', height: '2px', borderTop: '2px dashed #E60000', opacity: 0.3, pointerEvents: 'none', zIndex: 10 }
};
